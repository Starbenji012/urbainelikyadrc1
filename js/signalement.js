...
const SIGNALEMENTS_DELETE_ENDPOINTS = buildApiEndpoints(
  "signalements/delete.php",
);
...
function getSignalementTypeMeta(typeValue) {
  ...
  const map = {
    ...
    electricite: {
      key: "electricite",
      label: "Electricité",
      iconUrl: iconBasePath + "icons8-%C3%A9lectricit%C3%A9-32.png",
      size: [32, 32],
    },
    insecurite: {
      key: "insecurite",
...
function renderSignalementTypeBadgeHTML(typeValue, baseClass) {
  const meta = getSignalementTypeMeta(typeValue);
  const iconMarkup = meta.iconUrl
    ? `<img src="${meta.iconUrl}" alt="${meta.label}" class="${baseClass}-icon">`
    : "";
  return `<span class="${baseClass} type-${meta.key}">${iconMarkup}${meta.label}</span>`;
}
...
function renderMap() {
  ...
  visibleSignalements.forEach((sig) => {
    ...
    const photoMarkup = sig.photo
      ...
      : "";
    const typeBadgeMarkup = renderSignalementTypeBadgeHTML(
      sig.type,
      "popup-signalement-type",
    );
    const statusBadgeMarkup = renderSignalementStatusBadgeHTML(
      "popup-signalement-status",
    );
    ...
    const marker = L.marker([sig.lat, sig.lng], markerOptions).addTo(map)
      .bindPopup(`
        <div class="popup-signalement">
          ...
          ${photoMarkup}
          <p class="popup-badges">${typeBadgeMarkup}${statusBadgeMarkup}</p>
