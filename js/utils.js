...
function buildApiEndpoints(relativeApiPath) {
  const normalized = String(relativeApiPath || "").replace(/^\/+/, "");
  ...
  if (protocol === "http:" || protocol === "https:") {
    endpoints.unshift(`${window.location.origin}/backend/api/${normalized}`);
  }
  ...
  if (isLocalFrontend) {
    ...
    // Les navigateurs externes utilisent souvent un serveur PHP local distinct.
    for (const host of localHosts) {
      for (const port of ports) {
        endpoints.push(`http://${host}:${port}/backend/api/${normalized}`);
        endpoints.push(`http://${host}:${port}/api/${normalized}`);
      }
    }
  }
...
function unwrapApiListResponse(payload) {
...
const AUTH_LOGOUT_ENDPOINTS = buildApiEndpoints("auth/logout.php");
const AUTH_ME_ENDPOINTS = buildApiEndpoints("auth/me.php");
...
function initMenuBurger() {
  ...
  if (menuBurger && navigationMenu) {
    ...
    // sauf sur "Connecté" qui ouvre le panneau identité.
    navigationMenu.querySelectorAll("a").forEach((link) => {
      link.addEventListener("click", () => {
        const isAuthToggle = link.classList.contains("btn-connexion");
        const isConnected =
          String(localStorage.getItem("auth_connected") || "") === "1";
        if (isAuthToggle && isConnected) {
          return;
        }
...
function readAuthState() {
  const cachedProfile = window.__authProfile || null;
  ...
  const cachedName = String(cachedProfile?.nom || "").trim();
  const cachedEmail = String(cachedProfile?.email || "").trim();
  const cachedId = String(cachedProfile?.id || "").trim();
  const connected =
    Boolean(cachedProfile) ||
    String(localStorage.getItem("auth_connected") || "") === "1" ||
    Boolean(nom || email || userId);

  return {
    connected,
    nom: cachedName || nom || cachedEmail || email || AUTH_NAV_TEXT.defaultUser,
    email: cachedEmail || email,
    userId: cachedId || userId,
  };
}

function refreshAuthNavFromStorage() {
  ...
  const dropdown = linkItem?.querySelector(".auth-dropdown-panel") || null;
...
function initAuthNav() {
  ...
  if (typeof syncAuthStateFromBackend === "function") {
    syncAuthStateFromBackend().then(() => {
      refreshAuthNavFromStorage();
    });
  }
}
  ...
  for (const endpoint of AUTH_ME_ENDPOINTS) {
    try {
      ...
      window.__authProfile = {
        id: String(user.id || ""),
        nom: displayName || String(user.email || AUTH_NAV_TEXT.defaultUser),
      ...
      if (displayName) localStorage.setItem("user_nom", displayName);
      if (user.email) localStorage.setItem("user_email", String(user.email));
