...
function loadTemoignages() {
  ...
  if (temoignages.length === 0) {
    container.innerHTML =
      '<p style="text-align: center; padding: 20px; color: #666;">Aucun témoignage pour le moment. Soyez le premier à partager !</p>';
    return;
  }
...
function initTemoignageForm() {
  ...
  if (form) form.style.display = "none"; // masquer au chargement
  ...
  form.addEventListener("submit", async (ev) => {
    ...
    if (!message) {
      if (feedback) {
        ...
        feedback.textContent = "Le message est requis.";
      }
    ...
    if (feedback) {
      feedback.style.color = "green";
      feedback.textContent = "Merci ! Votre témoignage a été ajouté.";
    }
